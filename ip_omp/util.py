...
def get_data(transform, dataset):
    if dataset == "imagenet":
        train_ds = torchvision.datasets.DatasetFolder(
            str(MODULE_DIR / "data/ImageNet_clip/train"),
        ...
        test_ds = torchvision.datasets.DatasetFolder(
            str(MODULE_DIR / "data/ImageNet_clip/val"),
            loader=lambda path: np.load(path, allow_pickle=True),
            is_valid_file=lambda path: path[-4:] == ".npy",
        )
    ...
    if dataset == "places365":
        train_ds = custom_dataset.Places365_clip(
            root=str(MODULE_DIR / "data/Places365"),
    ...
    if dataset == "cifar10":
        train_ds = torchvision.datasets.CIFAR10(
            root=str(MODULE_DIR / "data"), train=True, download=True, transform=transform
        )
        test_ds = torchvision.datasets.CIFAR10(
            root=str(MODULE_DIR / "data"), train=False, download=True, transform=transform
        )

    if dataset == "cifar100":
        train_ds = torchvision.datasets.CIFAR100(
            root=str(MODULE_DIR / "data"), train=True, download=False, transform=transform
        )
        test_ds = torchvision.datasets.CIFAR100(
            root=str(MODULE_DIR / "data"), train=False, download=False, transform=transform
        )
